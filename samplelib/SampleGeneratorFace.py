...
# Use relative imports for siblings in samplelib
from .SampleGeneratorBase import SampleGeneratorBase
from .SampleLoader import SampleLoader
from .SampleProcessor import SampleProcessor  # Import the main class
from .Sample import SampleType  # Import SampleType enum from Sample.py
'''
arg
output_sample_types = [
    ...
    def batch_func(self, param ):
        ...
        while True:
            ...
            for n_batch in range(bs):
                ...
                try:
                    if SampleGeneratorFace._profiler_run_count < 1:  # Profile only the first time this block is hit
                        pr = cProfile.Profile()
                        pr.enable()

                        # Original call to SampleProcessor.process
                        x, = SampleProcessor.process ([sample], self.sample_process_options, self.output_sample_types, self.debug, ct_sample=ct_sample)
                        
                        pr.disable()
                        
                        s = std_io.StringIO()
                        sortby = 'cumulative'  # Sort by cumulative time spent in function and its sub-functions
                        ps = pstats.Stats(pr, stream=s).sort_stats(sortby)
                        
                        print(f"\n--- cProfile output for SampleProcessor.process (run {SampleGeneratorFace._profiler_run_count + 1}) ---")
                        ps.print_stats(50)  # Print the top 50 lines
                        print(s.getvalue())
                        print(f"--- End of cProfile output for SampleProcessor.process ---\n")
                        
                        SampleGeneratorFace._profiler_run_count += 1
                    else:
                        # Normal call without profiling for subsequent samples
                        x, = SampleProcessor.process ([sample], self.sample_process_options, self.output_sample_types, self.debug, ct_sample=ct_sample)
                except:
