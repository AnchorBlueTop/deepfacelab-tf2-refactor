...
class Sample(object):
    ...
    def get_xseg_mask(self):
        mask_buf = self.xseg_mask_compressed
        if mask_buf is None:
            return self.xseg_mask
        try: # Add try-except for robust decoding
            # Attempt to load directly as grayscale
            img = cv2.imdecode(mask_buf, cv2.IMREAD_GRAYSCALE)
            if img is None: # Fallback to unchanged and then convert if needed
                img = cv2.imdecode(mask_buf, cv2.IMREAD_UNCHANGED)
                if img is None:
                    print(f"Warning: Failed to decode xseg_mask for {self.filename}")
                    return None # Corrupt data?
                # Explicitly handle channel conversion if not grayscale
                if img.ndim == 3:
                    if img.shape[-1] == 4: # Handle BGRA/RGBA
                        # Prefer alpha channel as mask if it seems valid, otherwise convert BGR
                        alpha = img[..., 3]
                        if np.any(alpha < 255) and np.any(alpha > 0): # Check if alpha has variation
                             img = alpha
                        else: # Use color conversion if alpha is plain
                             img = cv2.cvtColor(img[..., :3], cv2.COLOR_BGR2GRAY)
                    elif img.shape[-1] == 3: # Handle BGR
                        img = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)
                    elif img.shape[-1] == 1: # Already H,W,1
                        img = img[..., 0] # Make it H,W
                    else: # Unexpected channel count
                         print(f"Warning: xseg_mask for {self.filename} has unexpected shape {img.shape}")
                         return None
                # If img.ndim is already 2 (H, W), it's grayscale
            # Ensure it's 2D before adding the final channel dimension
            if img.ndim != 2:
                 print(f"Warning: Could not reduce xseg_mask for {self.filename} to 2D. Shape: {img.shape}")
                 return None
            # Add channel dimension: (H, W) -> (H, W, 1)
            img = img[..., None]
        except Exception as e:
             print(f"Error decoding xseg_mask for {self.filename}: {e}")
             return None
        # Final check and type conversion
        if img.ndim != 3 or img.shape[-1] != 1:
             print(f"Warning: Final xseg_mask for {self.filename} has unexpected shape {img.shape}")
             return None
